...
contract Functionalities is DSTestPlus {
    ...
    function addYieldToken(address yieldToken, address adapter) public {
        IAlchemistV2AdminActions.YieldTokenConfig memory config =
            IAlchemistV2AdminActions.YieldTokenConfig({adapter: adapter, maximumLoss: 1, maximumExpectedValue: 1e50, creditUnlockBlocks: 1});
    ...
    */
    function setScenario(address caller, address proxyOwner, address[] calldata userList, uint96[] calldata debtList, uint96[] calldata overCollateralList)
        public
    {
    ...
    */
    function createCDPs(address[] calldata userList, uint96[] calldata debtList, uint96[] calldata overCollateralList) public {
    ...
    */
    function createCDP(address user, uint96 debt, uint96 overCollateral) public {
        ...
        if (underlyingBalance > 0) {
            alchemist.depositUnderlying(fakeYieldToken, underlyingBalance, user, minimumAmountOut(underlyingBalance, fakeYieldToken));
        }
    ...
    */
    function calculateBalance(uint256 debt, uint256 overCollateral, address underlyingToken) public returns (uint256) {
        IAlchemistV3.UnderlyingTokenParams memory params = alchemist.getUnderlyingTokenParameters(underlyingToken);
    ...
    */
    function assignYieldTokenToUser(address user, address token, uint256 amount) public {
    ...
    */
    function setRepayAmount(address user, address fakeUnderlyingToken, uint96 amount) public returns (uint256) {
        // Get repay limit
