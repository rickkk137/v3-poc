...
interface IMigrationTool {
    ...
    function previewMigration(address account, address startingYieldToken, address targetYieldToken, uint256 shares)
        external
        view
        returns (bool canMigrate, string memory state, uint256 amountToAdjust, uint256 minReturnShares, uint256 minReturnUnderlying);
    ...
    function migrateVaults(address startingYieldToken, address targetYieldToken, uint256 shares, uint256 minReturnShares, uint256 minReturnUnderlying)
        external
        returns (uint256 finalShares);
}
