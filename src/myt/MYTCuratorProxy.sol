...
// MYTOSX is a Minimal contract that allows only the admin (e.g. DAO) to modify the absolute and relative caps of an myt strategy
contract MYTCuratorProxy is IMYTCuratorProxy, PermissionedProxy {
    ...
    function _submitSetStrategy(address adapter, address myt) internal {
        IVaultV2 vault = IMYTVault(myt).MYT();
    ...
    function _vault(address adapter) internal returns (IVaultV2) {
        ...
        return IMYTVault(adapterToMYT[adapter]).MYT();
    }
}
